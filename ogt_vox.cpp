...
static bool _vox_get_vector_3_rotation_bits(const vector_3& vec, uint8_t& out_index) {
    const float* f = &vec.x;
    out_index = 3;
...
static uint8_t _vox_make_packed_rotation_from_transform(const ogt_vox_transform* transform) {
    ...
    vector_3 row0 = vector_3_make(transform->m00, transform->m10, transform->m20);
    vector_3 row1 = vector_3_make(transform->m01, transform->m11, transform->m21);
    vector_3 row2 = vector_3_make(transform->m02, transform->m12, transform->m22);
    uint8_t row0_index = 3, row1_index = 3, row2_index = 3;
    bool row0_negative = _vox_get_vector_3_rotation_bits(row0, row0_index);
    bool row1_negative = _vox_get_vector_3_rotation_bits(row1, row1_index);
    bool row2_negative = _vox_get_vector_3_rotation_bits(row2, row2_index);
...
static ogt_vox_transform _vox_make_transform_from_dict_strings(const char* rotation_string, const char* translation_string) {
    ...
    if (rotation_string != NULL) {
        ...
        vector_3 row0 = k_vectors[row0_vec_index];
        vector_3 row1 = k_vectors[row1_vec_index];
        vector_3 row2 = k_vectors[row2_vec_index];
        if (packed_rotation_bits & (1 << 4))
            row0 = vector_3_negate(row0);
        if (packed_rotation_bits & (1 << 5))
            row1 = vector_3_negate(row1);
        if (packed_rotation_bits & (1 << 6))
            row2 = vector_3_negate(row2);
